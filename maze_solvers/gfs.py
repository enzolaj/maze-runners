...
class Maze:
    ...
    Uses a A* Greedy-First Search algorithm.
    """
...
class Robot:
    """
    AI agent using Greedy Best-First Search (heuristic-driven).
    """
    def __init__(self, maze):
        ...
        # Add start cell to frontier with heuristic
        heapq.heappush(self.frontier, (self.heuristic(self.start_pos), self.start_pos))
    ...
    def step(self):
        if (self.x, self.y) == self.goal_pos or not self.frontier:
            return  # Goal reached or no moves left

        # Pop next cell from frontier
        _, (cx, cy) = heapq.heappop(self.frontier)
        self.x, self.y = cx, cy
        self.path.append((self.x, self.y))

        # Update knowledge
        valid_moves = self.maze.get_valid_moves(self.x, self.y)
        self.known_maze[(self.x, self.y)] = valid_moves
        ...
        for move in valid_moves:
            ...
            if neighbor not in self.visited:
                heapq.heappush(self.frontier, (self.heuristic(neighbor), neighbor))
                self.visited.add(neighbor)
                self.came_from[neighbor] = (self.x, self.y)

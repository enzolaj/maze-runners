...
import random, collections, heapq

# Kruskal's Mininum Spanning Tree:
# Generates a maze with weighed edges 
# and only one possible path with the losest cost(weight)
# Still using A* for robot
...
class Maze:
    """
    Maze generation using Kruskal's algorithm with weights.
    Passable walls have finite weight, unpassable walls = infinite.
    """
    ...
    def generate(self):
        # Initialize walls as edges
        edges = []
        for y in range(self.height):
            for x in range(self.width):
                if x < self.width - 1:  # East wall
                    weight = random.randint(1, 10)
                    edges.append(((x, y), (x + 1, y), weight))
                if y < self.height - 1:  # South wall
                    weight = random.randint(1, 10)
                    edges.append(((x, y), (x, y + 1), weight))
        ...
        for u, v, w in edges:
            if self.union(parent, rank, u, v):
                ...
                if x1 == x2:
                    if y1 < y2:
                        self.grid[(x1, y1)].add('S')
                        self.grid[(x2, y2)].add('N')
                    else:
                        self.grid[(x1, y1)].add('N')
                        self.grid[(x2, y2)].add('S')
                else:
                    if x1 < x2:
                        self.grid[(x1, y1)].add('E')
                        self.grid[(x2, y2)].add('W')
                    else:
                        self.grid[(x1, y1)].add('W')
                        self.grid[(x2, y2)].add('E')
                # Store weight for passable walls
                self.weights[(u, v)] = w
                self.weights[(v, u)] = w  # bidirectional
...
class Robot:
    """
    Robot using A* with weighted edges from the maze.
    """
    def __init__(self, maze):
        ...
        self.open_set = [(self.heuristic(self.start_pos), self.start_pos)]
    ...
    def heuristic(self, cell):
        return abs(cell[0] - self.goal_pos[0]) + abs(cell[1] - self.goal_pos[1])

    def step(self):
        if not self.open_set or (self.x, self.y) == self.goal_pos:
            return
        ...
        for move in self.known_maze[current]:
            nx, ny = current
            if move == 'N': ny -= 1
            elif move == 'S': ny += 1
            elif move == 'W': nx -= 1
            ...
            neighbor = (nx, ny)
            weight = self.maze.weights.get((current, neighbor), float('inf'))
...
class MazeApp:
    ...
    def _draw_maze_on_canvas(self, canvas, maze, show_full_maze, agent):
        ...
        # Draw weights
        if show_full_maze:
            for ((x, y), (nx, ny)), weight in maze.weights.items():
                if (x, y) < (nx, ny):
                    mx = (x + nx + 2) / 2 * CELL_SIZE
                    my = (y + ny + 2) / 2 * CELL_SIZE
                    canvas.create_text(mx - CELL_SIZE/2, my - CELL_SIZE/2,
                                    text=str(weight),
                                    fill="white")
